...
const OrderFormModal = ({onClose, selectedOrder}: OrderFormModalProps) => {
    const router = useRouter();

    const {register, handleSubmit, reset, formState: {errors, isSubmitting}} = useForm<
        OrderFormInput, unknown, OrderFormValues
    >({
    ...
    const onSubmit = async (data: OrderFormValues) => {
        let fetchBody, url
        if (selectedOrder) {
            url = `http://localhost:3001/orders/${selectedOrder.id}`
            fetchBody = {
                method: "PUT",
                headers: {
            ...
            >
                ...
                <div className="flex items-start justify-between border-b border-zinc-100 px-6 py-5">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-zinc-500">
                            {selectedOrder ? 'Edit order' : 'Add a new task to your project board.'}
                        </p>
                    </div>
                ...
                <form className="space-y-5 px-6 py-6" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="flex justify-end gap-3 border-t border-zinc-100 pt-5">
                        ...
                        >
                            {isSubmitting ? 'Processing....' : selectedOrder ? 'Edit Order' : 'Create Order'}
                        </button>
                    </div>
                </form>
            </div>
